...
### Resource check
Fetch the Resources page (using its ID from Phase 0). If it's still the unfilled stub (placeholders like `[number]` / `[e.g., ...]`), tell the user: *"Your Resources page is empty. Evaluation will use generic assumptions — scores will be less tailored. Want to fill it in now (5 min) or proceed?"* Wait for their answer. Don't block; their choice.
